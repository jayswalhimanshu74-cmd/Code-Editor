    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                .csrf(csrf -> csrf
                        ...
                        .ignoringRequestMatchers("/api/auth/**", "/ws/**", "/oauth2/**", "/login/oauth2/**", "/actuator/**", "/swagger-ui/**", "/v3/api-docs/**", "/swagger-ui.html")
                )
                .cors(cors -> cors.configurationSource(corsConfigurationSource()))
                .headers(headers -> {
                        headers.contentSecurityPolicy(csp -> csp.policyDirectives("default-src 'self'; connect-src 'self' ws: wss: http: https:; img-src 'self' data: https:; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-inline' 'unsafe-eval'"));
